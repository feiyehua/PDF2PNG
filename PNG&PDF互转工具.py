        ...
        if self.pdf_name:
            if self.pdf_name[-4:].lower() == '.pdf':
                ...
                for pg in range(0, pdf.pageCount):
                    ...
                    pm.save(path + os.sep + base_name[:-4] + '_' + '{:0>3d}.{}'.format(pg+1, 'png'))  # 保存图片
                pdf.close()
    ...
    def img2pdf(self):
        ...
        if self.file_dir:
            ...
            for img in sorted(glob.glob(img)):  # 排序获得对象
                ...
                doc.insert_pdf(img_pdf)  # 将单个文件插入到文档
                img_doc.close()
                img_pdf.close()
            ...
            if self.pdf_name[-4:].lower() == '.pdf':
                ...
                try:
                    ...
                    for pg in range(0, pdf.page_count):
                        ...
                        pm.save(path + os.sep + base_name[:-4] + '_' + '{:0>3d}.png'.format(pg + 1))  # 保存图片
                    pdf.close()
                    ...
                    for img in sorted(glob.glob(img)):  # 排序获得对象
                        img_doc = fitz.open(img)  # 获得图片对象
                        pdf_bytes = img_doc.convert_to_pdf()  # 获得图片流对象
                        img_pdf = fitz.open("pdf", pdf_bytes)  # 将图片流创建单个的PDF文件
                        doc.insert_pdf(img_pdf)  # 将单个文件插入到文档
                        img_doc.close()
                        img_pdf.close()
